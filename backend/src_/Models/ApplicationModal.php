...
class ApplicationModal
{
    ...
    public static function getPreviewApplicationById(int $applicationId): ?array
    {
        try {
            ...
            $row = $stmt->fetch(PDO::FETCH_ASSOC);
            $result = $row ? json_decode($row['fn_application_preview'], true) : null;
            return  $result ?: null;
        } catch (PDOException $e) {
            return null;
        }
    ...
    public static function applicationsubmitById(int $applicationId, string $submittedBy): ?array
    {
        try {
            ...
            $submittedByJson = json_encode([
                'submitted_by' => $submittedBy
            ]);

            $stmt = Database::WriteConnection()->prepare(
                'CALL public.sp_application_submit(:application_id::jsonb,:submitted_by::jsonb)'
            );

            $stmt->bindValue(':application_id', $applicationJson, PDO::PARAM_STR);
            $stmt->bindValue(':submitted_by', $submittedByJson, PDO::PARAM_STR);
    ...
    /**
     * Submit application with post/court selections via sp_application_submit.
     *
     * @param array<int, array<string, mixed>> $selections
     * @return array{ok: bool, message?: string, error?: string, application_id?: int|null}
     */
    public static function submitApplicationSelections(int $applicationId, array $selections): array
    {
        if ($applicationId <= 0) {
            return ['ok' => false, 'error' => 'application_id is required.'];
        }
        ...
        foreach ($selections as $sel) {
            ...
            $postId = (int) ($sel['post_id'] ?? $sel['postId'] ?? 0);
            $courtId = (int) ($sel['court_id'] ?? $sel['courtId'] ?? 0);
            $court = strtolower(trim((string) ($sel['court'] ?? '')));

            if ($court === '' && !empty($sel['benchKey'])) {
                $court = strtolower((string) $sel['benchKey']) === 'madurai' ? 'b' : 'a';
            }
            if ($court === 'madras' || $court === 'high court') {
                $court = 'a';
            }
            if ($court === 'madurai' || $court === 'madurai bench') {
                $court = 'b';
            }

            if ($postId <= 0 || $courtId <= 0 || !in_array($court, ['a', 'b'], true)) {
                return [
                    'ok' => false,
                    'error' => 'Each selection must include post_id, court (a/b), and court_id.',
                ];
            }
        ...
        return self::callJsonProcedure('sp_application_submit', $payload);
    }
}
